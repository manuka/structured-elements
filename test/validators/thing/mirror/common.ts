...
type Expectation = ({
  ...
  subject: unknown
  validator: StructuredElements.Validator<Thing, "mirror">
}) => void
...
const curryRunTest = ({
  ...
  return (testArgs) => {
    ...
    const collection = [thing0, thing1, thing2].reduce<
    ...
    const validator = Test.Modelling.validator(`Thing`, `mirror`)
...
export const setupValidatorThingMirrorTest = ({
  expectation,
}: {
  expectation: Expectation
}) => {
